...
export default function InteriorPaintingServicePage() {
  ...
  return (
    <main className="min-h-screen">
      ...
      <PageIntro
        ...
        description={t("serviceInteriorDescription")}
      />

      <section className="mx-auto grid w-full max-w-6xl gap-6 px-4 pb-24 pt-10 md:grid-cols-[1fr,1.1fr] md:px-6">
        <Image src={INTERIOR_IMAGE_URL} alt="Interior painting service" width={960} height={640} className="h-72 w-full rounded-3xl border border-theme object-cover shadow-sm md:h-full" />
        <article className="rounded-3xl border border-theme bg-theme-surface p-7 shadow-sm">
          <p className="text-base text-theme-soft">{t("serviceInteriorBodyOne")}</p>
          <p className="mt-4 text-base text-theme-soft">{t("serviceInteriorBodyTwo")}</p>

...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await createGalleryItem(body);
    return NextResponse.json({ data, message: t("api.gallery.item.created") }, { status: 201 });
  } catch (error) {
    ...
    return NextResponse.json({ statusCode: 500, error: "Internal Server Error", message: t("api.gallery.item.unexpectedCreate") }, { status: 500 });
  }
}
